<template>
  <main class="page">
    <h1 class="title">Pointcut tracer bullet — Vue + Webpack</h1>
    <p class="lede">
      Click <strong>pick</strong> (bottom-right), then click any element below.
      Your editor opens at that element's exact spot in this file — the Source
      Stamp ran as a webpack pre-loader, and the Bridge auto-attached via
      <code>devServer.setupMiddlewares</code>. Sending a pick to your Agent runs
      the same <code>createBridge()</code> handler the Vite demo uses.
    </p>
